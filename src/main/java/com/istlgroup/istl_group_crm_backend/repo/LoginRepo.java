...
public interface LoginRepo extends JpaRepository<LoginEntity,Long> {
	...
	// ── FIX #1: resolve manager name by id ──────────────────────────────────
	@Query("SELECT u.name FROM LoginEntity u WHERE u.id = :id")
	public Optional<String> findNameById(@Param("id") Long id);
	// ─────────────────────────────────────────────────────────────────────────
	...
	@Query("SELECT c FROM LoginEntity c WHERE " +
		       "(c.user_id = :username OR c.email = :username OR c.phone = :username)")
		LoginEntity findByUserId(@Param("username") String username);

	@Query("SELECT c FROM LoginEntity c WHERE c.email = :email")
