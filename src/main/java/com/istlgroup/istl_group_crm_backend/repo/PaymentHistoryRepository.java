...
public interface PaymentHistoryRepository extends JpaRepository<PaymentHistoryEntity, Long> {
 ...
 // Add to PaymentHistoryRepository.java

    List<PaymentHistoryEntity> findByInvoiceIdAndReceiptId(Long invoiceId, Long receiptId);
    
 // Add to PaymentHistoryRepository.java

    /**
     * Find all payment history by receipt ID
     */
    List<PaymentHistoryEntity> findByReceiptId(Long receiptId);

    /**
     * Delete all payment history by receipt ID
     */
    @Modifying
    @Query("DELETE FROM PaymentHistoryEntity ph WHERE ph.receiptId = :receiptId")
