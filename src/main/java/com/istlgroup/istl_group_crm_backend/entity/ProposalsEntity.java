...
public class ProposalsEntity {
    ...
    @Column(name = "offline_pdf_path", length = 512)
    private String offlinePdfPath;

    @Column(name = "offline_pdf_name", length = 255)
    private String offlinePdfName;

    // ── BLOB storage for new uploads ─────────────────────────────────────────
    @Lob
    @Column(name = "offline_pdf_data", columnDefinition = "MEDIUMBLOB")
    private byte[] offlinePdfData;
