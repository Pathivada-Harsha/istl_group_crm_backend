...
/**
 * One facility limit of a sanction's overall Limit — e.g. a Rs. 270 Cr
 ...
 */
@Entity
@Table(name = "sanction_limits")
@Data
public class SanctionLimitEntity {
    ...
    @Column(name = "limit_order", nullable = false)
    private Integer limitOrder;

    /**
     * This one facility limit's own amount — distinct from
     * {@code BorrowerSanctionEntity.limitAmount}, which is the sanction's
     * overall Limit that every row here must sum up to.
     */
    @Column(name = "facility_limit_amount", nullable = false)
    private BigDecimal facilityLimitAmount;

    /**
     * The user-facing name for this limit — "Fund Based Limit" for order 0,
     ...
     * {@code limitOrder}, never trusted from the client), so it always matches
     * the row's own position even though the row itself is fully replaced on
     * every save (see {@code persistSanctionLimits}). See
    ...
    /**
     * This limit's own JSON array of per-period repayment percentages — same
     * shape and same meaning as {@code BorrowerSanctionEntity.repaymentProfileJson}
     * (see that field's own comment), just scoped to this one limit's own
